  ...
  const handler = async (req, res) => {
    try {
      ...
      if (req.method === 'PUT') {
        ...
        // The bound is on the document, not on the JSON envelope around it, so a
        // draft of exactly MAX_BYTES is still accepted.
        const bytes = Buffer.byteLength(text, 'utf8')
        if (bytes > MAX_BYTES) {
          writeError(res, 413, 'too-large', `draft document is ${bytes} bytes; the limit is ${MAX_BYTES}`)
          return
        }
      ...
      // An over-long request body is the client's problem, not a server fault.
